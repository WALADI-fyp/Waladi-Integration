...
def parse_args() -> argparse.Namespace:
    ...
    # RGB source
    p.add_argument("--camera-url", default="auto", help="JPEG snapshot URL, 'auto' tries http://localhost:8001/snapshot first, 'none' opens Picamera2 directly")
    p.add_argument("--rgb-width", type=int, default=2304, help="Picamera2 capture width; default matches your camera_service")
    p.add_argument("--rgb-height", type=int, default=1296, help="Picamera2 capture height; default matches your camera_service")
    p.add_argument("--output-width", type=int, default=1280, help="Resize saved outputs to this width; 0 keeps original")
    p.add_argument("--settle-sec", type=float, default=2.0, help="Camera settle time before capture")
    p.add_argument("--warmup-frames", type=int, default=8, help="Discard this many camera frames before saving")
    p.add_argument("--jpeg-quality", type=int, default=90, help="Saved JPEG quality")
    p.add_argument("--debug-camera", action="store_true", help="Print camera metadata if using Picamera2")
...
def capture_rgb_picamera(width: int, height: int, settle_sec: float, warmup_frames: int = 8, debug: bool = False) -> np.ndarray:
    try:
    ...
    try:
        ...
        # Match your camera_service: use the full sensor field of view.
        try:
        ...
        # Let AE/AWB/focus settle and discard early frames.
        last = None
        for _ in range(max(1, int(warmup_frames))):
            last = picam2.capture_array("main")
            time.sleep(0.05)
        ...
        try:
            ...
            pass
        rgb = picam2.capture_array("main") if last is None else last
        if debug:
            print(f"[rgb] metadata: {meta}")
...
def main() -> None:
    ...
    if camera_url == "auto":
        ...
        try:
            rgb = fetch_rgb_from_url(camera_url)
            print(f"[rgb] used existing camera_service: {camera_url}")
        except Exception as e:
            print(f"[rgb] camera_service not available ({e}); opening Picamera2 directly")
            rgb = capture_rgb_picamera(args.rgb_width, args.rgb_height, args.settle_sec, args.warmup_frames, args.debug_camera)
    elif camera_url:
        rgb = fetch_rgb_from_url(camera_url)
        print(f"[rgb] used URL: {camera_url}")
    else:
        rgb = capture_rgb_picamera(args.rgb_width, args.rgb_height, args.settle_sec, args.warmup_frames, args.debug_camera)
    rgb = rotate_image(rgb, args.rotate_rgb)
    if args.output_width and rgb.shape[1] != args.output_width:
        scale = args.output_width / float(rgb.shape[1])
        rgb = cv2.resize(rgb, (args.output_width, max(1, int(rgb.shape[0] * scale))), interpolation=cv2.INTER_AREA)
